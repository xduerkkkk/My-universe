...
> **【组长解析】**
> 
> 这一部分是你们代码逻辑的文字版。**答辩防坑点**：老师可能会问“你们的m序列是怎么生成的？” 你们要能答出是通过“6级移位寄存器”和“异或操作”生成的，因为 $2^6 - 1 = 63$，正好符合题目要求的 63 位周期。
...
_（此处插入截图：源码信号时域波形 与 已调信号时域波形）_
...
**问题描述**：题目要求源码信号为周期 63 bits 的 m 序列 。但在进行 QPSK 调制时，需要将串行数据进行 2 比特的并串转换（分为 I 路和 Q 路）。由于 63 是奇数，无法被 2 整除，如果直接映射，会导致最后一个比特缺失配对，无法构成完整的 QPSK 符号。 **解决办法**：在编写 M 代码时，我们采用了序列拼接策略。将生成的一个周期的 63 位 m 序列与自身拼接，构成了一个长度为 126 bits 的双周期序列。这样既保留了 m 序列本身的伪随机特性和频谱特性，又将其长度转换为了偶数，完美解决了 QPSK 符号完整映射的矛盾。
...
> **【组长解析】** 别忘了，这部分**必须手写拍照插入** ，千万别直接打印文字，否则不符合规定会扣分！
...
> **【组长解析】** 这样写分工，每个人都沾了“技术”的边，完美避开了“不能只负责文档工作”的雷区 。记得让大家把名字签在白纸上，拍照发给你，你裁剪一下贴到 Word 里。
> 
