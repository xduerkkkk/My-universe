...
在原始 Transformer 中，位置是**加**进去的：
$$q_{new} = q_{word} + P_m$$
$$k_{new} = k_{word} + P_n$$

当我们做点积计算分数时，会发生什么？
$$Score = (q_{word} + P_m) \cdot (k_{word} + P_n)$$
展开后有四项：
$$Score = \underbrace{q \cdot k}_{\text{纯语义}} + \underbrace{q \cdot P_n}_{\text{语义x位置}} + \underbrace{P_m \cdot k}_{\text{位置x语义}} + \underbrace{P_m \cdot P_n}_{\text{纯位置}}$$

**问题来了：**
虽然第四项 $P_m \cdot P_n$ 确实包含了相对距离信息（正弦公式的设计），但是！中间那两项（交叉项）包含了 $P_m$ 和 $P_n$ 的**绝对值**。
这意味着，**计算结果里，“绝对位置”的信息像杂质一样混在里面，去不掉！**

所以，模型必须费劲去训练，学会在权重里抵消掉这些绝对位置的干扰，才能提取出相对关系。这就不是“天然”的。

#### 2. RoPE（旋转）的魔法
在 RoPE 中，位置是**乘**（旋转）进去的：
$$q_{new} = R_m \times q_{word}$$
$$k_{new} = R_n \times k_{word}$$
(其中 $R_m$ 代表旋转 $m\theta$ 度)

当我们做点积时，奇迹发生了（利用复数或向量旋转的数学性质）：
$$Score = (R_m q) \cdot (R_n k) = q \cdot (R_n - R_m) \cdot k$$
**数学结论是：** 两个旋转向量的点积，等于**它们旋转角度之差**的旋转。
$$Score = q \cdot R_{(n-m)} \cdot k$$

**看！公式里的 $m$（绝对位置）和 $n$（绝对位置）彻底消失了！只剩下了 $(n-m)$（相对距离）。**





RoPE的核心，就在于，不是加法“加进去”句子向量的。
我们回忆一下，原先的位置编码怎么用的？是QKV计算完，产出最终的句子向量后，加进去的。（啊？不对！居然是Q向量、K向量、V向量形成时就加入位置编码吗？？为啥）

而RoPE，是在计算注意力分数，Q点积K时，融入“位置”这个信息的。
...
当我们做点积时，利用复数或向量旋转的数学性质可以得到：
$$Score = (R_m q) \cdot (R_n k) = q \cdot (R_n - R_m) \cdot k= q \cdot R_{(n-m)} \cdot k$$
即，两个旋转向量的点积，等于**它们旋转角度之差**的旋转。
**看！公式里的 $m$（绝对位置）和 $n$（绝对位置）彻底消失了！只剩下了 $(n-m)$（相对距离）
